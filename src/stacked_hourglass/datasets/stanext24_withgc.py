...
from configs.dog_breeds.dog_breed_class import COMPLETE_ABBREV_DICT, COMPLETE_SUMMARY_BREEDS, SIM_MATRIX_RAW, \
    SIM_ABBREV_INDICES
...
class StanExtGC(data.Dataset):
    ...
    def __init__(self, opts, image_path=None, is_train=True, inp_res=256, out_res=64, sigma=1,
        ...
        self.train_dict, init_test_dict, init_val_dict = utils_stanext.load_stanext_json_as_dict(split_train_test=True,
                                                                                                 V12=self.V12)
        ...
        for name in self.test_name_list:
            if name in self.gc_annots_categories.keys():
                ...
                if (value['is_vis'] in [True, None]) and (value['is_flat'] in [True, None]) and (
                not value['pose'] == 'cantsee'):
        ...
        self.path_anipose_out_root = os.path.join(STANEXT_RELATED_DATA_ROOT_DIR,
                                                  'animalpose_hg8_v1_results_on_StanExt')  # this is from hg_anipose_after01bugfix_v1
            ...
            for img_name in self.train_name_list:
                ...
                if not (folder_name in breed_dict):
                    breed_dict[folder_name] = {
                        ...
                        'index': breed_index}
                    breed_index += 1
            with open(breed_json_path, 'w', encoding='utf-8') as f:
                json.dump(breed_dict, f, ensure_ascii=False, indent=4)
        else:
            with open(breed_json_path) as json_file:
                breed_dict = json.load(json_file)
        return breed_dict
    ...
    def __getitem__(self, index):
        ...
        if self.load_dinov2_features:
            ...
            if dino_idx is not None:
                ...
                if hasattr(self, 'dino_patch_start_indices') and hasattr(self, 'dino_num_patches_per_image') and \
                        dino_idx < len(self.dino_patch_start_indices) and dino_idx < len(
                    self.dino_num_patches_per_image):
        ...
        if gc_isflat:
            if name.split('.')[0] in self.gc_annots_overview_stage3:
                ...
                gc_vertdists_overview = self.gc_annots_overview_stage2b_contact[name.split('.')[0]][
                ...
                if 'pose' in self.gc_annots_categories[name]:
                    ...
                    if pose_label in ['standing_4paws']:
                        gc_vertdists_overview = self.gc_annots_overview_stages12_all4pawsincontact['all4pawsincontact'][
                            'gc_vertdists_overview']
                        gc_info_tch = torch.tensor(
                            gc_vertdists_overview[:, :])  # torch.tensor(gc_vertdists_overview[:, 0])
                        gc_info_available = True
                        gc_touching_ground = True
        ...
        try:
            ...
            with open(anipose_res_path) as f:
        ...
        for i in range(nparts):
            ...
            if tpts[i, 1] > 0:
                tpts[i, 0:2] = to_torch(
                    transform(tpts[i, 0:2] + 1, c, s, [self.out_res, self.out_res], rot=r, as_int=False)) - 1
        ...
        meta = {'index': index, 'center': c, 'scale': s,
                ...
                'ind_dataset': 0,
                'dinov2_features': dinov2_feature_vector}  # ind_dataset=0 for stanext or stanexteasy or stanext 2
