...
from stacked_hourglass.datasets.utils_dataset_selection import get_evaluation_dataset, get_sketchfab_evaluation_dataset, \
    get_crop_evaluation_dataset, get_norm_dict
...
from combined_model.loss_utils.loss_utils import leg_sideway_error, leg_torsion_error, tail_sideway_error, \
    tail_torsion_error, spine_torsion_error, spine_sideway_error
...
def main(args):
    ...
    root_out_path = os.path.join(os.path.dirname(path_model_file_complete).replace(cfg.paths.ROOT_CHECKPOINT_PATH,
                                                                                   cfg.paths.ROOT_OUT_PATH + 'results_ttopt/'),
                                 out_sub_name)
    ...
    remeshing_path = os.path.join(root_data_path, 'smal_data_remeshed', 'uniform_surface_sampling',
                                  'my_smpl_39dogsnorm_Jr_4_dog_remesh4000_info.pkl')

    loss_weight_path = os.path.join(os.path.dirname(__file__), '../', 'src', 'configs', 'ttopt_loss_weights',
                                    args.loss_weight_ttopt_path)
    ...
    if cfg.data.DATASET == 'sketchfab':
        ...
        val_dataset, val_loader, len_val_dataset, test_name_list, stanext_data_info, stanext_acc_joints = get_sketchfab_evaluation_dataset(
            cfg.optim.BATCH_SIZE, args.workers)
    elif cfg.data.DATASET == 'ImgCropList':
        input_folder = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'test_image_crops')
        val_dataset, val_loader, len_val_dataset, test_name_list, stanext_data_info, stanext_acc_joints = get_crop_evaluation_dataset(
            cfg.optim.BATCH_SIZE, args.workers, input_folder)
    elif cfg.data.DATASET in ['stanext24_easy', 'stanext24', 'stanext24_withgc', 'stanext24_withgc_big']:
        print('load dataset')
        val_dataset, val_loader, len_val_dataset, test_name_list, stanext_data_info, stanext_acc_joints = get_evaluation_dataset(
            cfg.data.DATASET, cfg.data.VAL_OPT, cfg.data.V12, cfg.optim.BATCH_SIZE, args.workers)
    else:
    ...
    logscale_part_list = SMAL_MODEL_CONFIG[smal_model_type][
        'logscale_part_list']  # ['legs_l', 'legs_f', 'tail_l', 'tail_f', 'ears_y', 'ears_l', 'head_l']
    smal = SMAL(smal_model_type=smal_model_type, template_name='neutral', logscale_part_list=logscale_part_list).to(
        device)
    ...
    remeshing_relevant_faces = torch.tensor(remeshing_dict['smal_faces'][remeshing_dict['faceid_closest']],
    ...
    for i, (input, target_dict) in iterable:
        ...
        for key in target_dict.keys():
            ...
            elif key in ['index', 'pts', 'tpts', 'target_weight', 'silh', 'silh_distmat_tofg', 'silh_distmat_tobg',
        ...
        all_pose_6d = rotmat_to_rot6d(res['pose_rotmat'][:, None, 1:, :, :].clone().reshape((-1, 3, 3))).reshape(
            (bs, -1, 6))  # [bs, 34, 6]
        all_orient_6d = rotmat_to_rot6d(res['pose_rotmat'][:, None, :1, :, :].clone().reshape((-1, 3, 3))).reshape(
            (bs, -1, 6))  # [bs, 1, 6]
        ...
        for ind_img in range(input.shape[0]):
            ...
            pred_unp_prep = torch.cat(
                (res['hg_keyp_256'][ind_img, :, :].detach(), res['hg_keyp_scores'][ind_img, :, :]), 1)
            inp_img = input[ind_img, :, :, :].detach().clone()
            out_path = root_out_path + name + '_hg_key.png'
            save_input_image_with_keypoints(inp_img, pred_unp_prep, out_path=out_path, threshold=0.01,
                                            print_scores=True, ratio_in_out=1.0)  # threshold=0.3
            # save the input image
            img_inp = input[ind_img, :, :, :].clone()
            for t, m, s in zip(img_inp, stanext_data_info.rgb_mean, stanext_data_info.rgb_stddev): t.add_(
                m)  # inverse to transforms.color_normalize()
            ...
            target_img_silh = Image.fromarray(
                np.uint8(255 * target_dict['silh'][ind_img, :, :].detach().cpu().numpy())).convert('RGB')
            ...
            hg_img_silh = Image.fromarray(
                np.uint8(255 * res['hg_silh_prep'][ind_img, :, :].detach().cpu().numpy())).convert('RGB')
            ...
            optimed_orient_6d = all_orient_6d[ind_img, None, :, :].to(device).clone().detach().requires_grad_(
                True)  # [1, 1, 6]
            ...
            optimed_camera_flength = res['flength'][ind_img, None, :].to(device).clone().detach().requires_grad_(
                True)  # [1,1]
            ...
            optimizer = torch.optim.SGD(
                ...
                [optimed_camera_flength, optimed_trans_z, optimed_trans_xy, optimed_pose_6d, optimed_orient_6d,
                 optimed_betas, optimed_betas_limbs],
                lr=5 * 1e-4,  # 1e-3,
                momentum=0.9)
            optimizer_vshift = torch.optim.SGD(
                [optimed_camera_flength, optimed_trans_z, optimed_trans_xy, optimed_pose_6d, optimed_orient_6d,
                 optimed_betas, optimed_betas_limbs, optimed_vert_off_compact],
                lr=1e-4,  # 1e-4,
                momentum=0.9)
            nopose_optimizer = torch.optim.SGD(
                # [optimed_pose, optimed_trans_xy, optimed_betas, optimed_betas_limbs, optimed_orient, optimed_vert_off_compact],
                [optimed_camera_flength, optimed_trans_z, optimed_trans_xy, optimed_orient_6d, optimed_betas,
                 optimed_betas_limbs],
                lr=5 * 1e-4,  # 1e-3,
                momentum=0.9)
            ...
            with torch.no_grad():
                ...
                weights_resh = kp_weights[ind_img, None, :, :].reshape(
                    (-1))  # target_dict['tpts'][:, :, 2].reshape((-1))
                keyp_w_resh = keypoint_weights.repeat((batch_size, 1)).reshape((-1))
                # prepare predicted ground contact labels
                sm = nn.Softmax(dim=1)
                target_gc_class = sm(res['vertexwise_ground_contact'][ind_img, :, :])[None, :,
                                  1]  # values between 0 and 1
                target_gc_class_remeshed = torch.einsum('ij,aij->ai', remeshing_relevant_barys,
                                                        target_gc_class[:, remeshing_relevant_faces].to(device=device,
                                                                                                        dtype=torch.float32))
            ...
            for i in loop:
                ...
                if i == 0:
                    ...
                    if losses["lapctf"]['weight_vshift'] > 0.0:
                        ...
                        smal_downsampling_npz_name = 'mesh_downsampling_' + os.path.basename(
                            SMAL_MODEL_CONFIG[smal_model_type_downsampling]['smal_model_path']).replace('.pkl',
                                                                                                        '_template.npz')
                ...
                smal_verts, keyp_3d, _ = smal(beta=optimed_betas, betas_limbs=optimed_betas_limbs,
                ...
                pred_silh_images, pred_keyp_raw = silh_renderer(vertices=smal_verts, points=keyp_3d, faces=faces_prep,
                                                                focal_lengths=optimed_camera_flength)
                pred_keyp = pred_keyp_raw[:, :24, :]

                # save silhouette reprojection visualization
                if i == 0:
                    img_silh = Image.fromarray(
                        np.uint8(255 * pred_silh_images[0, 0, :, :].detach().cpu().numpy())).convert('RGB')
                    img_silh.save(root_out_path_details + name + '_silh_ainit.png')
                    my_mesh_tri = trimesh.Trimesh(vertices=smal_verts[0, ...].detach().cpu().numpy(),
                                                  faces=faces_prep[0, ...].detach().cpu().numpy(), process=False,
                                                  maintain_order=True)
                ...
                losses['keyp']['value'] = ((((output_kp_resh - target_kp_resh)[weights_resh > 0] ** 2).sum(
                    axis=1).sqrt() * \
                                            weights_resh[weights_resh > 0]) * keyp_w_resh[weights_resh > 0]).sum() / \
                                          max((weights_resh[weights_resh > 0] * keyp_w_resh[weights_resh > 0]).sum(),
                                              1e-5)
                ...
                # ground contact loss
                sel_verts = torch.index_select(smal_verts, dim=1, index=remeshing_relevant_faces.reshape((-1))).reshape(
                    (batch_size, remeshing_relevant_faces.shape[0], 3, 3))
                ...
                gc_errors_plane, gc_errors_under_plane = calculate_plane_errors_batch(verts_remeshed,
                                                                                      target_gc_class_remeshed_prep,
                                                                                      isflat, istouching)
                ...
                if (losses["edge"][current_weight_name] + losses["normal"][current_weight_name] + losses["laplacian"][
                    current_weight_name]) > 0:
                ...
                total_loss = 0.0
                for k in ['keyp', 'silhouette', 'pose_legs_side', 'pose_legs_tors', 'pose_tail_side', 'pose_tail_tors',
                          'pose_spine_tors', 'pose_spine_side', 'gc_plane', 'gc_belowplane', 'edge', 'normal',
                          'laplacian', 'arap', 'lapctf']:
                ...
                if i % 150 == 0:
                    ...
                    # save image overlay
                    visualizations = silh_renderer.get_visualization_nograd(smal_verts, faces_prep,
                                                                            optimed_camera_flength, color=0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-lwttopt', '--loss-weight-ttopt-path', default='bite_loss_weights_ttopt.json', type=str,
                        metavar='PATH',
    ...
    # --- Add DINOv2 related arguments ---
    parser.add_argument('--use_dinov2_features', type=lambda x: bool(strtobool(x)), default=False,
                        help='Enable DINOv2 features for inference (True/False)')
    parser.add_argument('--dinov2_model_name', type=str, default='facebook/dinov2-base',
                        help='Name of DINOv2 model from Hugging Face')
    parser.add_argument('--dinov2_pca_model_path', type=str, default=None,
                        help='Path to DINOv2 PCA model and scaler .pkl file (REQUIRED if use_dinov2_features is True)')
